...
class App extends Component {
  ...
  renderFoods = (foodArray, searchValue) => {
    ...
    const filteredAray = foodArray.filter(food => {
      return food.name.toLowerCase().includes(searchValue.toLowerCase())
    })
    return filteredAray.map((food, index) => 
    <FoodBox key={index} food={food} handleAddToList={handleAddToList} />
    )
  }
  ...
  return (
    <main>
      ...
      <button className="btn is-info" onClick={handleClickAdd}>
        Add New Food
      </button>
